# -*- coding: utf-8 -*-
"""
Created on Thu Jun 30 15:37:28 2022
...
class PointProgressionFrameProcedure(ProgressionFrameProcedure):
    ...
    def compute(self,acq,folder):
        self.m_marker=f"{folder}:LHEE"
...
class PelvisProgressionFrameProcedure(ProgressionFrameProcedure):
    ...
    def __init__(self,marker=",",frontMarkers=[","], backMarkers=[","]):
    ...
    def compute(self,acq,folder):
        ...
        self.m_marker=f"{folder}:LFWT"

        self.m_frontmarkers = [f"{folder}:LFWT",f"{folder}:RFWT"]
        self.m_backmarkers = [f"{folder}:LBWT",f"{folder}:RBWT"]
